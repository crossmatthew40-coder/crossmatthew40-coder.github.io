(() => {
  ...
  function compactTokens(t){
    ...
    return [t.buttonRadius,t.cardRadius,t.logoRadius,t.borderWidth,t.headingWeight,t.headingTransform,t.letterSpacing,t.sectionGap,t.density];
  }
  function hosted(value){ return /^https?:\/\//i.test(String(value||'')) ? String(value) : ''; }
  function compact(c, reduced=false){
    const t=c.theme||{};
    const fontChoice = t.fontChoice && t.fontChoice !== 'auto' ? t.fontChoice : '';
  ...
  function slug(){
    const raw=q('slug')?.value||'new-brand';
    return typeof cleanSlug==='function'?cleanSlug(raw):raw.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'');
  }
  function snapshotUrl(reduced=false){
    const pkg=getPackage();
    const payload=base64Url(JSON.stringify(compact(pkg.config,reduced)));
    return `${location.origin}/cards/?brand=${encodeURIComponent(pkg.slug||slug())}#c=${payload}`;
  }
  async function ensureQr(){
    if(window.qrcode) return;
    await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src='https://cdn.jsdelivr.net/npm/qrcode-generator@1.4.4/qrcode.min.js';s.onload=resolve;s.onerror=()=>reject(new Error('QR library unavailable'));document.head.appendChild(s);});
  }
  function svgFor(url,colour){
    let qr=null,last=null;
    for(const level of ['H','Q','M','L']){
      try{const candidate=window.qrcode(0,level);candidate.addData(url);candidate.make();qr=candidate;break;}catch(e){last=e;}
    }
    if(!qr) throw last||new Error('Card link is too large for a QR code');
    const count=qr.getModuleCount(),margin=4,total=count+margin*2;let paths='';
    for(let r=0;r<count;r++)for(let c=0;c<count;c++)if(qr.isDark(r,c))paths+=`M${c+margin},${r+margin}h1v1h-1z`;
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${total} ${total}" shape-rendering="crispEdges"><rect width="100%" height="100%" fill="#fff"/><path d="${paths}" fill="${colour}"/></svg>`;
  }
  async function generate(silent=false){
    if(!silent){q('workingQrRefresh').disabled=true;q('workingQrRefresh').textContent='Generating…';}
    try{
      await ensureQr();
      let url=snapshotUrl(false),svg='';
      try{svg=svgFor(url,q('workingQrColour').value||'#000');}
      catch(e){url=snapshotUrl(true);svg=svgFor(url,q('workingQrColour').value||'#000');q('workingQrState').textContent='QR created from the core card details; gallery links were omitted to keep the QR reliable.';q('workingQrState').classList.add('warn');}
      currentUrl=url;currentSvg=svg;
      q('workingQrUrl').value=url;q('workingQrDestination').textContent=url;q('workingQrImage').innerHTML=svg;q('workingQrTitle').textContent=(q('businessName')?.value?.trim()||'Customer')+' QR Code';q('workingQrOutput').classList.add('show');
      if(!q('workingQrState').classList.contains('warn')){q('workingQrState').textContent='QR is linked to the current finished design and ready to test.';}
      if(!silent&&typeof showStatus==='function')showStatus('Working QR created for this finished card.');
  ...
  q('workingQrOpen').addEventListener('click',()=>{if(currentUrl)window.open(currentUrl,'_blank','noopener');else generate(true).then(()=>window.open(currentUrl,'_blank','noopener'));});
  q('workingQrRefresh').addEventListener('click',()=>generate(false));
  q('workingQrColour').addEventListener('input',schedule);
  q('workingQrPng').addEventListener('click',downloadPng);q('workingQrSvg').addEventListener('click',downloadSvg);q('workingQrCopy').addEventListener('click',copy);
  editor.addEventListener('input',e=>{if(!section.contains(e.target))schedule();});
  editor.addEventListener('change',e=>{if(!section.contains(e.target))schedule();});
  editor.addEventListener('click',e=>{if(section.contains(e.target))return;if(e.target.closest('button'))setTimeout(schedule,90);});

  window.highStyleWorkingCardUrl = () => currentUrl || snapshotUrl(false);
  setTimeout(()=>generate(true),420);
})();
