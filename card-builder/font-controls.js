(() => {
  ...
  const choices = {
    auto: {
      label: 'Auto from website',
      description: 'Follows the branding scan automatically.',
      heading: SYSTEM,
      body: SYSTEM
    },
    modern: {
    ...
    luxury: {
      label: 'Luxury',
      description: 'Elegant high-end serif headings.',
      heading: 'Didot, "Bodoni 72", Georgia, "Times New Roman", serif',
      body: SYSTEM
    },
    editorial: {
      label: 'Editorial',
      description: 'Refined hospitality and lifestyle feel.',
      heading: 'Baskerville, Georgia, "Times New Roman", serif',
      body: '"Helvetica Neue", Arial, sans-serif'
    },
    bold: {
      label: 'Bold',
      description: 'High-impact modern branding.',
    ...
    rounded: {
      ...
      heading: '"Avenir Next Rounded", "Trebuchet MS", "Segoe UI", Arial, sans-serif',
      body: '"Avenir Next", "Segoe UI", Arial, sans-serif'
    },
    classic: {
      label: 'Classic',
      description: 'Traditional and established.',
  ...
  function autoKey(){
    ...
    if (/luxury|premium|understated/.test(hay)) return 'luxury';
    if (/editorial|refined/.test(hay)) return 'editorial';
    if (/bold|modern/.test(hay)) return 'bold';
    if (/soft|warm|welcoming|rounded/.test(hay)) return 'rounded';
    return 'modern';
  }

  function plan(key = selected){
    const resolved = key === 'auto' ? autoKey() : key;
    return {...(choices[resolved] || choices.modern), key:resolved, mode:key};
  }
  ...
  function inferSavedChoice(theme){
    ...
    if (label.includes('luxury') || label.includes('elegant serif')) return 'luxury';
    if (label.includes('editorial')) return 'editorial';
    if (label.includes('bold')) return 'bold';
    if (label.includes('rounded') || label.includes('warm')) return 'rounded';
    if (label.includes('classic')) return 'classic';
    return 'auto';
  }
  ...
  updatePreview = function(c){
    baseUpdatePreview(c);
    const p = c?.theme?.headingFont && c?.theme?.bodyFont
      ? {heading:c.theme.headingFont, body:c.theme.bodyFont}
      : plan();
    ...
    document.querySelectorAll('.preview-btn,.chip,.preview-role,.preview-tagline,.preview-location,.preview-person,.preview-footer,.preview-label,.preview-eyebrow,.tripadvisor-preview-link').forEach(el => {
      el.style.fontFamily = p.body;
    });
  };
  ...
  style.textContent = `
    ...
    .font-choice-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.font-choice{position:relative;text-align:left;border:1px solid #2e2e2e;border-radius:14px;background:#121212;color:#fff;padding:12px;cursor:pointer;min-height:82px;transition:.18s ease}
    ...
    @media(max-width:640px){.font-choice-grid{grid-template-columns:1fr}}
  `;
  document.head.appendChild(style);

  const panel = document.createElement('section');
  panel.className = 'font-controls-card';
  panel.id = 'fontControls';
  panel.innerHTML = `
    <h3>Font Styles</h3>
    <p>Choose the typography for this customer card. Auto keeps following the website branding scan.</p>
    <div class="font-choice-grid" id="fontChoiceGrid"></div>
    <div class="font-current" id="fontCurrent"></div>`;

  const contactHead = [...document.querySelectorAll('.section-head')].find(el => /contact/i.test(el.textContent));
  const editor = document.querySelector('.editor-panel');
  if (contactHead?.parentNode) contactHead.parentNode.insertBefore(panel, contactHead);
  ...
  Object.entries(choices).forEach(([key,item]) => {
    ...
    button.dataset.fontChoice = key;
    const previewFont = key === 'auto' ? SYSTEM : item.heading;
    button.innerHTML = `<span class="font-choice-name">${item.label}</span><span class="font-choice-desc">${item.description}</span>`;
    button.querySelector('.font-choice-name').style.fontFamily = previewFont;
    button.addEventListener('click', () => {
      selected = key;
      renderSelected();
      update();
      if (typeof showStatus === 'function') showStatus(key === 'auto' ? 'Font set to Auto — it will follow the website branding.' : `${item.label} font style applied.`);
    });
  ...
  ['designMood','smartDesignStyle','designType'].forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
    new MutationObserver(() => {
      if (selected === 'auto') { renderSelected(); update(); }
    }).observe(el,{childList:true,subtree:true,characterData:true});
  });

  const branding = document.getElementById('applyWebsiteDesign');
  if (branding) branding.addEventListener('click', () => setTimeout(() => { renderSelected(); update(); }, 0));

  try {
    const current = baseGetConfig();
    selected = inferSavedChoice(current?.theme || {});
  } catch(e) {}

  window.highStyleFontControls = {
    select(key){ if (choices[key]) { selected = key; renderSelected(); update(); return true; } return false; },
    get(){ return {selected, ...plan()}; },
