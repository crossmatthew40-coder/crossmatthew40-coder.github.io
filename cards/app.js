(function(){
  ...
  function addAction(label,value,href,primary){
    if(!href)return;
    const a=document.createElement('a');a.className='action'+(primary?' primary':'');a.href=href;
    ...
    a.querySelector('.action-label').textContent=label;a.querySelector('.action-value').textContent=value;
  ...
  if(brand.phone||brand.email||brand.website){
    ...
    save.addEventListener('click',function(e){
      ...
      const blob=new Blob([lines.join('\r\n')],{type:'text/vcard'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=((brand.businessName||'contact').replace(/[^a-z0-9]+/gi,'-').replace(/^-|-$/g,'')||'contact')+'.vcf';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);
    });
  }
  ...
  [['Instagram',brand.instagram,brand.instagramLabel||'Instagram'],['LinkedIn',brand.linkedin,'LinkedIn'],['TikTok',brand.tiktok,'TikTok']].forEach(([name,url,label])=>{
    if(!url)return;const a=document.createElement('a');a.className='social';a.href=url;a.target='_blank';a.rel='noopener';a.textContent=label||name;socials.appendChild(a)
  });
  $('socialSection').classList.toggle('hidden',!socials.children.length);

  const services=$('services');(brand.services||[]).forEach(s=>{const el=document.createElement('span');el.className='chip';el.textContent=s;services.appendChild(el)});
  $('servicesSection').classList.toggle('hidden',!services.children.length);

  $('review').textContent=brand.review||'';$('reviewSection').classList.toggle('hidden',!brand.review);
  const gallery=$('gallery');(brand.gallery||[]).forEach(src=>{const img=document.createElement('img');img.src=src;img.alt='';img.loading='lazy';gallery.appendChild(img)});
