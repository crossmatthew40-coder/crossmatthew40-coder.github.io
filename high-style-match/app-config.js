...
(function applyHighStyleBranding(){
  ...
  function ensureOpening(){const style=document.createElement('style');style.textContent=`
    ...
    .hsm-opening-logo{
      animation:hsmLogoSlide .55s cubic-bezier(.16,1,.3,1) both!important;
    ...
    .hsm-opening-logo>.brandmark{
      grid-column:1!important;
      width:clamp(170px,19vw,260px)!important;
      height:clamp(170px,19vw,260px)!important;
      max-width:34vh!important;
      max-height:34vh!important;
      flex:none!important;
    }
    .hsm-opening-brandcopy{
    ...
    @media(max-width:720px){
      .hsm-opening-logo{grid-template-columns:auto auto!important;column-gap:14px!important;width:98vw!important;padding:10px!important}
      .hsm-opening-logo>.brandmark{width:clamp(92px,28vw,138px)!important;height:clamp(92px,28vw,138px)!important}
      .hsm-opening-brandcopy strong{font-size:clamp(24px,7vw,38px)!important}
  ...
  function routeNewVisitorsAfterSplash(){
    ...
    if(!isMainEntry||sessionStorage.getItem('hsm_gate_seen')==='1')return;
    setTimeout(()=>{sessionStorage.setItem('hsm_gate_seen','1');location.href='./subscribe/'},5000)
  }
  function apply(){
    loadTheme();ensureOpening();
    const logoUrl=window.HSM_APP.logoUrl;
    document.querySelectorAll('.brandmark').forEach(mark=>{
      const opening=mark.closest('.hsm-opening-logo');
      mark.innerHTML='';
      Object.assign(mark.style,{background:'transparent',backgroundColor:'transparent',boxShadow:'none',border:'0',borderRadius:'0',overflow:'visible',padding:'0'});
      const img=document.createElement('img');img.src=logoUrl;img.alt='High Style';img.decoding='async';
      Object.assign(img.style,{width:'100%',height:'100%',display:'block',objectFit:'contain',objectPosition:'center',background:'transparent',border:'0',boxShadow:'none',transform:'none',filter:'none'});
      mark.appendChild(img);
      if(opening){
        opening.querySelectorAll('.hsm-opening-brandcopy').forEach(el=>el.remove());
        const copy=document.createElement('div');copy.className='hsm-opening-brandcopy';
        copy.innerHTML='<strong>High Style Match</strong><span>Part of the High Style Group</span>';
