...
window.HSM_APP = {
  version: '1.1.1-capture-compatibility',
...
(function applyHighStyleBranding(){
  function loadTheme(){
    if(!document.querySelector('link[data-hsm-pro-theme]')){const link=document.createElement('link');link.rel='stylesheet';link.href='./capture-one-theme.css?v=20260907-7';link.dataset.hsmProTheme='true';document.head.appendChild(link)}
    if(!document.querySelector('link[data-hsm-purple-theme]')){const purple=document.createElement('link');purple.rel='stylesheet';purple.href='./high-style-purple-theme.css?v=20260907-7';purple.dataset.hsmPurpleTheme='true';document.head.appendChild(purple)}
  }
  ...
  function apply(){loadTheme();ensureTwoSecondOpening();const logoUrl=window.HSM_APP.logoUrl;document.querySelectorAll('.brandmark').forEach(mark=>{const isOpeningLogo=!!mark.closest('.hsm-opening-logo');mark.innerHTML='';Object.assign(mark.style,{background:'transparent',backgroundColor:'transparent',boxShadow:'none',border:'0',borderRadius:'0',overflow:'visible',padding:'0'});if(isOpeningLogo){mark.style.width='clamp(180px, 24vw, 300px)';mark.style.height='clamp(58px, 8vw, 96px)';mark.style.flex='0 0 auto'}const img=document.createElement('img');img.src=logoUrl;img.alt='High Style';Object.assign(img.style,{width:'100%',height:'100%',display:'block',objectFit:'contain',objectPosition:'center',background:'transparent',border:'0',boxShadow:'none'});mark.appendChild(img)});let favicon=document.querySelector('link[rel="icon"]');if(!favicon){favicon=document.createElement('link');favicon.rel='icon';document.head.appendChild(favicon)}favicon.type='image/svg+xml';favicon.href=logoUrl;routeNewVisitorsAfterSplash()}
  ...
  function loadProductUpgrades(){loadScript('./product-upgrades.js?v=20260907-7','hsm-upgrades');loadScript('./capture-delivery-tools.js?v=20260907-7','hsm-capture-delivery');loadScript('./integration-notice.js?v=20260907-7','hsm-integration-notice')}
