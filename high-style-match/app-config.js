...
(function applyHighStyleBranding(){
  const BUILD='20260907-13';
  function loadTheme(){
    // The monochrome file is now the only runtime theme. Loading the old layered themes
    // caused extra requests, extra CSS parsing and repeated restyles on every startup.
  ...
  function ensureFastOpening(){const style=document.createElement('style');style.textContent=`
    ...
    .hsm-opening-logo{animation:hsmLogoSlide .36s cubic-bezier(.16,1,.3,1) both!important;display:flex!important;align-items:center!important;justify-content:center!important;gap:24px!important;flex-wrap:nowrap!important;padding:24px!important}
