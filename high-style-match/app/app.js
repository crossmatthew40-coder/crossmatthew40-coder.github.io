...
function renderShell(){
 ...
 [["dashboard","⌂","Home"],["projects","▢","Projects"],["library","◉","Library"],["clients","♙","Clients"],["ai","✦","AI Tools"],["exports","↑","Exports"],["settings","⚙","Settings"]].map(([r,i,n])=>'<button data-route="'+r+'" class="'+(S.route===r&&!S.project?"on":"")+'"><span class="ico">'+i+"</span><span>"+n+'</span>'+(r==="ai"?'<span class="soon">SOON</span>':"")+"</button>").join("")+
...
function renderRoute(){if(S.project)return renderProject();if(S.route==="dashboard")renderDashboard();else if(S.route==="projects")renderProjects();else if(S.route==="library")renderLibrary();else if(S.route==="clients")renderClients();else if(S.route==="settings")renderSettings();else if(S.route==="ai")renderAI();else renderExports()}
...
function renderProject(){
 ...
 const tabs=[["overview","Overview"],["upload","Upload"],["cull","Smart Cull"],["aiVision","AI Vision"],["best","Best Picks"],["review","Client Review"],["approved","Approved for Edit"],["editing","Editing"],["delivery","Final Delivery"]];
 C.innerHTML=pageHead(p.name,statusLabel(p.project_type)+" • "+statusLabel(p.mode)+" • "+statusLabel(p.status),'<button class="btn secondary" id="backProjects">← Projects</button>')+workflow(p)+'<div class="tabs">'+tabs.map(([id,n])=>'<button data-tab="'+id+'" class="'+(S.tab===id?"on":"")+'">'+n+(["cull","aiVision"].includes(id)?' · Soon':"")+"</button>").join("")+'</div><div id="projectBody"></div>';
 $("#backProjects").onclick=()=>navigate("projects");$$("[data-tab]").forEach(b=>b.onclick=()=>{S.tab=b.dataset.tab;renderProject()});
 if(S.tab==="overview")renderOverview();else if(S.tab==="upload")renderUpload();else if(S.tab==="best")renderBest();else if(S.tab==="review")renderReview();else if(S.tab==="approved")renderApproved();else if(S.tab==="editing")renderEditing();else if(S.tab==="delivery")renderDelivery();else renderComingSoon(S.tab==="cull"?"Smart Cull":"AI Vision")
}
...
function renderAI(){$("#content").innerHTML=pageHead("AI Tools","High Style Match intelligence will live here.")+'<section class="card panel"><div class="empty"><h3>AI Vision + Smart Cull — Coming Soon</h3><p>The product does not fake these actions. The cloud project system, secure review, editing handoff and final delivery are being made reliable first.</p><button class="btn secondary" disabled>Coming Soon</button></div></section>'}
function renderExports(){$("#content").innerHTML=pageHead("Exports","Project exports remain attached to the photography workflow.")+'<section class="card panel"><div class="empty"><h3>Exports are project-based</h3><p>Open a project to work with selected, approved and final photographs. CSV and filename-map export will be added here next.</p></div></section>'}
