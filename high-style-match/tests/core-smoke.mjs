...
const production=fs.readFileSync(path.join(root,'production-studio-tools.js'),'utf8');
...
assert.match(config,/production-studio-tools\.js/,'Production Studio is not loaded');
assert.match(production,/automaticGrouping/,'Automatic photo grouping is missing');
assert.match(production,/runConsistency/,'Edit consistency checker is missing');
assert.match(production,/Retouching notes/,'Retouching notes are missing');
assert.match(production,/Client approval/,'Client approval gallery is missing');
assert.match(production,/PRODUCTION REPORT/,'Branded production reports are missing');
